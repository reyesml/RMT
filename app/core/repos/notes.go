...
func (r noteRepo) ListAllByPersonId(pid uint) ([]models.Note, error) {
	var ns []models.Note
	result := r.db.Order("created_at desc").Where("person_id = ?", pid).Find(&ns)
	return ns, result.Error
}

func (r noteRepo) ListByPersonId(pid uint) ([]models.Note, error) {
	var ns []models.Note
	result := r.db.Order("created_at desc").Preload("PersonQuality").Where("person_id = ? and person_quality_id = 0", pid).Find(&ns)
	return ns, result.Error
}

func (r noteRepo) ListByPersonQualityId(pqid uint) ([]models.Note, error) {
	var ns []models.Note
	result := r.db.Order("created_at desc").Preload("PersonQuality").Preload("Person").Where("person_quality_id = ?", pqid).Find(&ns)
	return ns, result.Error
}
